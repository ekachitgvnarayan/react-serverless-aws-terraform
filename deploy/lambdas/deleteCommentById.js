...
const SERVICE_NAME = "DeleteClaimNote";
...
exports.handler = (event, context, callback) => {
  const noteId = event.path.split("/")[2];
  structuredLog("INFO", "Request received", event, { noteId });

  const params = {
    Key: { commentId: { S: noteId } },
    TableName: "comments"
  };
  ...
  ddb.deleteItem(params, function (err, data) {
    ...
    if (err) {
      ...
      structuredLog("INFO", "Claim note deleted", event, { noteId });
      responseBody = data;
    }
