...
const SERVICE_NAME = "DeleteApproval";
...
exports.handler = (event, context, callback) => {
  const approvalId = event.path.split("/")[2];
  structuredLog("INFO", "Request received", event, { approvalId });

  const params = {
    Key: { likeId: { S: approvalId } },
    TableName: "likes"
  };
  ...
  ddb.deleteItem(params, function (err, data) {
    ...
    if (err) {
      ...
      structuredLog("INFO", "Approval revoked", event, { approvalId });
      responseBody = data;
    }
